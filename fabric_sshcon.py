...
client = Connection(
                host = "",                                     #input ur remote host adds>
...
trans = client.put('manPurge.sh', remote='/bin')                #copy or send the bash script to />

print ("Uploaded {0.local} to {0.remote}".format(trans))

command = "sh /bin/manPurge.sh"                                 #running the bash script in remote>
